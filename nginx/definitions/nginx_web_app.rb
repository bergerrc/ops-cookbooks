define :nginx_web_app, :template => "site.erb", :enable => true do
  ...
  template "#{node[:nginx][:dir]}/ssl/#{application[:domains][0]}.crt" do
    cookbook 'nginx'
    mode '0600'
    source "ssl.key.erb"
    ...
    only_if do
      application[:ssl_support] && !application[:domains].nil
    end
  end

  template "#{node[:nginx][:dir]}/ssl/#{application[:domains][0]}.key" do
    cookbook 'nginx'
    mode '0600'
    source "ssl.key.erb"
  ...
  template "#{node[:nginx][:dir]}/ssl/#{application[:domains][0]}.ca" do
    cookbook 'nginx'
    mode '0600'
    source "ssl.key.erb"
    ...
    notifies :restart, "service[nginx]"
    only_if do
      application[:ssl_support] && application[:ssl_certificate_ca] && !application[:domains].nil
    end
  end

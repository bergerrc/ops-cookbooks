define :nginx_web_app, :template => "site.erb", :enable => true do
  ...
  template "#{node[:nginx][:dir]}/ssl/#{application_name}.crt" do
    cookbook 'nginx'
    mode '0600'
    source "ssl.key.erb"
  ...
  template "#{node[:nginx][:dir]}/ssl/#{application_name}.key" do
    cookbook 'nginx'
    mode '0600'
    source "ssl.key.erb"
  ...
  template "#{node[:nginx][:dir]}/ssl/#{application_name}.ca" do
    cookbook 'nginx'
    mode '0600'
    source "ssl.key.erb"
